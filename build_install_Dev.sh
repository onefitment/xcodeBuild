...
###############配置项目名称和路径等相关参数
projectName="MHCB2BApp" #项目所在目录的名称
isWorkSpace=true  #判断是用的workspace还是直接project，workspace设置为true，否则设置为false
projectDir=~/Desktop/mhcApp/bugatti #项目所在目录的绝对路径
buildConfig="Release" #编译的方式,默认为Release,C91Store,CWeb,还有Debug等

###############配置下载的文件名称和路径等相关参数
appName="MHCB2BApp"  #在网页上显示的名字
wwwIPADir=~/Documents/mhcipa_dev/ #html，ipa，icon，plist最后所在的目录绝对路径
url="https://img.maihaoche.com/ipa" #下载路径

PARAM_SIGNING_IDENTITY="iPhone Distribution: HangZhou yijiqingchen Information Technology Company Limited (A4VHY8L789)"
...
###############获取版本号,bundleID
infoPlist="$projectName/MHCB2BAppDev-Info.plist"
echo "infoPlist 名称" >>$infoPlist
...
if $isWorkSpace ; then  #判断编译方式
    ...
    xcodebuild  -workspace $projectName.xcworkspace -scheme MHCB2BAppDev  -configuration $buildConfig clean build SYMROOT=$buildAppToDir
else
    echo  "开始编译target...." >>$logPath
    xcodebuild  -target  MHCB2BAppDev  -configuration $buildConfig clean build SYMROOT=$buildAppToDir
fi
#判断编译结果
if test $? -eq 0
then
...
###############开始打包成.ipa
ipaName=`echo MHCB2BAppDev | tr "[:upper:]" "[:lower:]"` #将项目名转小写
#ipaName=`echo $projectName #项目名称暂时不转小写
appDir=$buildAppToDir/$buildConfig-iphoneos/  #app所在路径
echo "开始打包$projectName.app成$projectName.ipa....." >>$logPath
xcrun -sdk iphoneos PackageApplication -v $appDir/$ipaName.app -o $appDir/$ipaName.ipa  #将app打包成ipa
			...
			<dict>
	          ...
	          <string>$url/$ipaName.ipa</string>
					...
					<dict>
						...
						<key>url</key>
	          			<string>https://img.maihaoche.com/b2b57.png</string>
					</dict>
					<dict>
						<key>kind</key>
						<string>full-size-image</string>
						<key>url</key>
	          			<string>https://img.maihaoche.com/b2b512.png</string>
					</dict>
				</array>
				<key>metadata</key>
				<dict>
